...
#undef fail
    ...
    // Initialize everything from vertex coordinates, polygon vertices and polygon neighbors
    void Mesh::calculate_from_polygons()
    {
#define fail(message) std::cerr << message << std::endl; exit(1);
        for (int i = 0; i < mesh_vertices.size(); i++) {
            Vertex& v = mesh_vertices[i];
            v.obstacle_edge.clear();
            v.polygons.clear();
            v.is_turning_vertex = false;
        }
        for (int i = 0; i < mesh_polygons.size(); i++) {
            Polygon& p = mesh_polygons[i];
            p.edges.clear();
        ...
        for (int i = 0; i < mesh_polygons.size(); i++)
        {
            ...
            if (p.vertices.size() > max_poly_sides) {
                max_poly_sides = p.vertices.size();
            }

            bool found_trav = false;
            p.is_one_way = true;
            for (int j = 0; j < p.polygons.size(); j++)
            {
                int polygon_index = p.polygons[j];
                if (polygon_index == -1)
                    continue;
                if (found_trav)
                {
        ...
        for (int i = 0; i < mesh_vertices.size(); ++i) {
            ...
            for (int j = 0; j < mesh_vertices[i].polygons.size(); ++j) {
                ...
                for (int k = 0; k < mesh_polygons[current_poly].vertices.size(); ++k) {
                    if (mesh_polygons[current_poly].vertices[k] == i) {
                        if (k == 0) {
                            double angle = get_angles_between_points(mesh_vertices[i].p, start_point, 
                                mesh_vertices[mesh_polygons[current_poly].vertices[mesh_polygons[current_poly].vertices.size() - 1]].p);
                            temp_vertex.push_back(make_pair(angle, current_poly));
                            neighbour_info.insert({ current_poly,mesh_polygons[current_poly].polygons[k] });
                        }
                        else {
                            double angle = get_angles_between_points(mesh_vertices[i].p, start_point, 
                                mesh_vertices[mesh_polygons[current_poly].vertices[k - 1]].p);
                            temp_vertex.push_back(make_pair(angle, current_poly));
                            neighbour_info.insert({ current_poly,mesh_polygons[current_poly].polygons[k] });
                        }
                    }
...
bool Mesh::poly_point_location(int polygon, Point& p, PointLocation& pl)
{
    ...
    case PolyContainment::INSIDE:
        ...
        pl = { PointLocation::IN_POLYGON, polygon, -1, -1, -1 };
        return true;
        ...
        // This one lies on a corner.
    {
        ...
        if (v.is_corner)
        {
            if (v.is_ambig)
            {
                pl = { PointLocation::ON_CORNER_VERTEX_AMBIG, -1, -1,
                        result.vertex1, -1 };
            }
            else
            {
                pl = { PointLocation::ON_CORNER_VERTEX_UNAMBIG,
                        polygon, -1, result.vertex1, -1 };
            }
        }
        else
        {
            pl = { PointLocation::ON_NON_CORNER_VERTEX,
                    polygon, -1,
                    result.vertex1, -1 };
        }
        return true;
...
void Mesh::out_polym(vector<int>& vertices) {
    std::cout << "Polygon(";
    for (int i = 0; i < vertices.size(); i++) {
        std::cout << mesh_vertices[vertices[i]].p;
...
{
    return get_point_location(p, 0);
}

PointLocation Mesh::get_point_location(Point& p, int recursion_depth)
{
    ...
    PointLocation out_pl;
    while (i >= 0 && i < ps)
    {
        const int polygon = polys[i];
        if (poly_point_location(polygon, p, out_pl))
            return out_pl;
    ...
    if (recursion_depth < 4) {
        polyanya::Point rand_add = polyanya::Point{ (double)rand() / RAND_MAX, (double)rand() / RAND_MAX } * (1e-2 * pow(10, recursion_depth));
        polyanya::Point corrected = p + rand_add;
        std::cout << "At rec depth: " << recursion_depth << " added " << rand_add << " to " << p << std::endl;
        return get_point_location(corrected, recursion_depth + 1);
    }
...
void Mesh::print_mesh(std::ostream& outfile)
{
    outfile << "mesh\n";
    outfile << "2\n";
    outfile << mesh_vertices.size() << " " << mesh_polygons.size() << "\n";
    for (Vertex v : mesh_vertices)
    {
        outfile << v.p.x << " " << v.p.y << " ";
        outfile << v.polygons.size();
        for (int p : v.polygons)
        {
            outfile << " " << p;
        }
        outfile << "\n";
