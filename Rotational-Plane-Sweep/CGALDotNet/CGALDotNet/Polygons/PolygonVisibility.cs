...
namespace CGALDotNet.Polygons
{
    ...
    public sealed class PolygonVisibility<K> : PolygonVisibility where K : CGALKernel, new()
    {
        ...
        public IntPtr GetLocatorAndTEV(PolygonWithHoles2<K> polygon)
        {
            //CheckPolygon(polygon);
            return Kernel.GetLocatorAndTEV(polygon.Ptr);
        }

        public void ComputeVisibilityTEVCached(Point2d point, IntPtr triExpPtr, out PolygonWithHoles2<K> result)
        {
            //CheckPolygon(polygon);
            var ptr = Kernel.ComputeVisibilityTEVCached(point, triExpPtr);
            result = new PolygonWithHoles2<K>(ptr);
        }

        public void ComputeVisibilityTEVCached(Point2d point, IntPtr triExpPtr, List<Point2d> points)
        {
            points.Clear();
            ComputeVisibilityTEVCached(point, triExpPtr, out var res);
            res.GetBoundary().GetPoints(points);
        }

        public bool CdtContains(Point2d point, IntPtr triExpPtr)
        {
            return Kernel.CdtContains(point, triExpPtr);
        }

        public int FindIndexContainsPoint(Point2d point, IList<PolygonWithHoles2<K>> polygons)
        {
            for (int i = 0; i < polygons.Count; i++)
            {
                if (polygons[i].ContainsPoint(point))
                    return i;
            }
            return -1;
        }
