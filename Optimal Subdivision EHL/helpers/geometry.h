...
inline bool onSegmentApprox(const Point& p, const Point& q, const Point& r)
{
    if (q.x <= fmax(p.x, r.x) + EPSILON && q.x >= fmin(p.x, r.x) - EPSILON &&
        q.y <= fmax(p.y, r.y) + EPSILON && q.y >= fmin(p.y, r.y) - EPSILON)
        return true;
...
inline int orientation(const Point& p, const Point& q, const Point& r)
    {
        ...
        if (val > -1e-8 && val < 1e-8) return 0;  // colinear
...
// and 'p2q2' intersect.
 inline int doIntersect(const Point& p1, const Point& q1, const Point& p2, const Point& q2)
    {
    ...
    inline bool rayTolineIntersection(Point p1, Point p2, Point q1, Point q2, Point& intersection) {
        intersection = lineLineIntersection(p1, p2, q1, q2);
        if (intersection == Point{ -1,-1 }) {
            //not intersect
            if (get_orientation(q1, p1, q2) == Orientation::COLLINEAR) {
                if (q1 == p1 || q2 == p1)
                    return true;
                return q1.distance(p2) <= q1.distance(p1) + EPSILON ||
                    q2.distance(p2) <= q2.distance(p1) + EPSILON;
            }
            return false;
        }
        else {
