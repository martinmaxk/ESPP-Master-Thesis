...
#ifndef LEARNED_HEURISTIC_EBHL_POLY_QUERY_V2_H
#define LEARNED_HEURISTIC_EBHL_POLY_QUERY_V2_H
...
namespace polyanya {
    ...
    typedef EBHL_poly* EBHLPtrV2;
// Polyanya instance for point to point search
    class EBHL_poly_query_v2
    {
    private:
        const MeshPtr mesh;
    ...
    public:
        int num_via = 0;
        int num_visible_checks = 0;
        int num_via_pruned = 0;
        double visible_time = 0;
        warthog::timer visible_timer = warthog::timer();
        double location_time = 0;
        warthog::timer location_timer = warthog::timer();
        double inner_time = 0;
        warthog::timer inner_timer = warthog::timer();
        ...
        EBHL_poly_query_v2() = default;
        EBHL_poly_query_v2(MeshPtr m , EBHLPtrV2 eb,  vector<Point> tp, vector<Point> op) :  mesh(m), ebhl(eb), turning_point(tp), obstacle_middle_point(op){ init();}
        EBHL_poly_query_v2(EBHL_poly_query_v2 const &) = delete;
        void operator=(EBHL_poly_query_v2 const &x) = delete;
        ...
        bool checkNextPolygon(int poly_id_1, int poly_id_2,const polyanya::Point& start_p, 
            const polyanya::Point& end_p, const polyanya::PointLocation& end_p_pl,
            bool& intersects, polyanya::Point& intersection, bool stop_on_end){
            if(stop_on_end && poly_id_2 == end_p_pl.poly1){
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                const polyanya::Point right = mesh->mesh_vertices[last_vertex].p;
                polyanya::Point temp_intersection;
                if(rayTolineIntersection(start_p, end_p, left, right, temp_intersection))
                {
                    if(stop_on_end && (left == end_p || right ==end_p)){
                        return true;
                    }else  if(P[i] != -1 ){
                        if(checkNextPolygon(poly_id_2, P[i] , start_p, end_p, end_p_pl, 
                            intersects, intersection, stop_on_end)){
                            return true;
                        }
                    }
                    else {
                        if (temp_intersection == polyanya::Point{ -1, -1 }) {
                            last_vertex = this_vertex;
                            continue;
                        }
                        if (left == end_p || right == end_p)
                            temp_intersection = end_p;
                        else if (left == start_p || right == start_p) {
                            last_vertex = this_vertex;
                            continue;
                        }
                        else
                            temp_intersection = lineLineIntersection(start_p, end_p, left, right);
                        if (start_p.distance_sq(temp_intersection) > start_p.distance_sq(intersection))
        ...
        bool isVisible(const polyanya::Point& start_p, const polyanya::Point& end_p, 
            const polyanya::PointLocation& start_p_pl, const polyanya::PointLocation& end_p_pl,
            bool& intersects, polyanya::Point& intersection, bool stop_on_end = true){
            visible_search_id ++;
            intersection = start_p;
            return checkNextPolygon(-1, start_p_pl.poly1, start_p, end_p, end_p_pl, intersects, intersection, stop_on_end);
        ...
        bool search(){
            start_index  = ebhl->get_poly_index(start);
            goal_index  = ebhl->get_poly_index(goal);
            const PointLocation& start_pl = get_point_location_in_search(start, mesh, false);
            const PointLocation& goal_pl = get_point_location_in_search(goal, mesh, false);

            const unsigned& start_begin = ebhl->get_poly_labels_begin(start_index);
            const unsigned& target_begin = ebhl->get_poly_labels_begin(goal_index);
            distance = INF_WEIGHT;

            std::unordered_set<int>& start_visible = ebhl->poly_visible[start_index];
            std::unordered_set<int>& target_visible = ebhl->poly_visible[goal_index];

            if(isVisible(start,goal,start_pl,goal_pl)){
                distance = start.distance(goal);
                return true;
            }
            //go through the hubs and labels stored in both start and target poly
        ...
        inline void get_min_v_path(int hub_index, polyanya::Point p, std::unordered_set<int>& visible, 
            vector<int>& visibility_times_flag, vector<bool>& visibility_cache,
            double& distance, int& best_predecessor, int& best_convex) {
            unsigned convex_begin = ebhl->get_convex_begin(hub_index);
            unsigned convex_end = ebhl->get_convex_end(hub_index);
            /*if (v_cache.size() < (index + (convex_end - convex_begin)) / 32)
                v_cache.resize(v_cache.size() * 2);*/
            //check through all the distances of stored convex vertex to hub
            for (unsigned k = convex_begin; k != convex_end; ++k) {
                const Convex_vertices_label& cv = ebhl->get_convex_label(k);
                //if the visibility is true
                bool is_visible = cv.visibility;
                if (ebhl->all_fully_visible)
                    is_visible = true;
                else if (!ebhl->store_visibility) {
                    auto search = visible.find(cv.convex_vertex);
                    is_visible = search != visible.end();
                }
                num_via++;
                double tmp_distance = p.distance(turning_point[cv.convex_vertex])
                    + cv.distance;
                //if (cv.visibility) {
                if (is_visible) {
                    if (tmp_distance < distance) {
                        distance = tmp_distance;
                        best_predecessor = cv.predecessor;
                        best_convex = cv.convex_vertex;
                    }
                    ...
                    if (tmp_distance < distance) {
                        ...
                        if (visibility_times_flag[cv.convex_vertex] == visible_search_id) {
                            if (visibility_cache[cv.convex_vertex]) {
                                distance = tmp_distance;
                                best_predecessor = cv.predecessor;
                                best_convex = cv.convex_vertex;
                            }
                            ...
                            visibility_times_flag[cv.convex_vertex] = visible_search_id;
                            num_visible_checks++;
                            visibility_cache[cv.convex_vertex] = ebhl->check_visiblity(p,
                                obstacle_middle_point[cv.convex_vertex],
                                turning_point[cv.convex_vertex],
                                cv.convex_vertex
                            );
        ...
        inline void get_min_v_path_loop(int hub_index, polyanya::Point p, std::unordered_set<int>& visible,
            vector<int>& visibility_times_flag, vector<bool>& visibility_cache,
            double& distance, int& best_predecessor, int& best_convex) {
            int cur_hub_index = hub_index;
            int index = 0;
            if (ebhl->store_visibility) {
                get_min_v_path(cur_hub_index, p,
                    visible, visibility_times_flag, visibility_cache,
                    distance, best_predecessor, best_convex);
                return;
            }
            while (cur_hub_index != -1) {
                get_min_v_path(cur_hub_index, p, 
                    visible, visibility_times_flag, visibility_cache,
                    distance, best_predecessor, best_convex);
                cur_hub_index = ebhl->get_hub_parent(cur_hub_index);
            }
        }

        inline PointLocation poly_point_location(int child_index, Point& p) {
            /*PointLocation pl;
            bool found_loc = mesh->poly_point_location(ebhl->mesh_poly_parents[child_index], p, pl);
            if (pl.type == PointLocation::ON_CORNER_VERTEX_AMBIG)
            {
                pl.type = PointLocation::ON_CORNER_VERTEX_UNAMBIG;
                pl.poly1 = ebhl->mesh_poly_parents[child_index];
            }
            else if (!found_loc)
                return get_point_location_in_search(p, mesh, false);
            return pl;*/
            return { PointLocation::IN_POLYGON, ebhl->mesh_poly_parents[child_index], -1, -1, -1 };
        }
        ...
        bool search_path(){
            ...
            const PointLocation& start_pl = poly_point_location(start_index, start);
            const PointLocation& goal_pl = poly_point_location(goal_index, goal);
            location_timer.stop();
            location_time += location_timer.elapsed_time_micro();
            distance = INF_WEIGHT;

            std::unordered_set<int> temp;
            std::unordered_set<int>& start_visible = ebhl->store_visibility ? temp : ebhl->poly_visible[start_index];
            std::unordered_set<int>& target_visible = ebhl->store_visibility ? temp : ebhl->poly_visible[goal_index];
            best_hub_id = -1;
            visible_timer.start();
            bool is_visible = isVisible(start, goal, start_pl, goal_pl);
            visible_timer.stop();
            visible_time += visible_timer.elapsed_time_micro();
            if(is_visible){
                distance = start.distance(goal);
                return true;
            }
            ...
            //goes through all the labels stored in both start and target poly
            for (unsigned i = start_begin, j = target_begin; ; ) {
                int v1 =  ebhl->get_hub_id(i), v2 = ebhl->get_hub_id(j);
                if (v1 == numOfVertices) break;  // Sentinel
                //if v1 and v2 are the same hub
                if (v1 == v2) {
                    double lower_bound = ebhl->get_lower_bound(i) + ebhl->get_lower_bound(j);
                    if (lower_bound > distance) {
                        // Incorrect for compressed trees, does not count parents, just use numbers from uncompressed
                        num_via_pruned += ebhl->get_convex_end(i) - ebhl->get_convex_begin(i);
                        num_via_pruned += ebhl->get_convex_end(j) - ebhl->get_convex_begin(j);
                        i += 2;
                        j += 2;
                        continue;
                    }
                    ...
                    get_min_v_path_loop(i, start, start_visible,
                        start_visibility_times_flag, start_visibility_cache,
                        start_distance, best_start_predecessor, best_start_convex);

                    double target_distance = INF_WEIGHT;
                    int best_target_predecessor = -1;
                    int best_target_convex = -1;
                    get_min_v_path_loop(j, goal, target_visible,
                        goal_visibility_times_flag, goal_visibility_cache,
                        target_distance, best_target_predecessor, best_target_convex);
            ...
            inner_timer.stop();
            inner_time += inner_timer.elapsed_time_micro();
