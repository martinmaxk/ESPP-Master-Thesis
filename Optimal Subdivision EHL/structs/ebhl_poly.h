...
#ifndef LEARNED_HEURISTIC_EBHL_POLY_H
#define LEARNED_HEURISTIC_EBHL_POLY_H
...
namespace polyanya{
    ...
    public:
        MeshPtr mp;
        bool store_visibility;
        bool all_fully_visible;
        int lambda;
        int compression = 0;
        double cost_ratio;
        unsigned long hub_space = 0;
        unsigned long via_space = 0;
        unsigned long aux_space = 0;
        std::vector<unsigned> poly_label_begin;
        std::vector<int> poly_parents;
        std::vector<std::unordered_set<int>> poly_visible;
        std::vector<unsigned> hub_label_begin;
        std::vector<double> hub_lower_bound;
        std::vector<int> hub_parents;

        std::vector<int> mesh_poly_parents;

        std::vector<Poly_label> poly_labels;
        ...
        EBHL_poly(MeshPtr mp) {
            this->mp = mp;
            poly_label_begin.clear();
            poly_parents.clear();
            poly_visible.clear();
         ...
         */
        void initialize_poly_map(int num_polys = -1) {
            int prev_num_labels = poly_labels.size();
            if (num_polys == -1)
                num_polys = mp->mesh_polygons.size();
            poly_labels.resize(num_polys);
            for (int i = prev_num_labels; i < num_polys; ++i)
                poly_labels[i].poly = i;
            poly_parents.resize(num_polys);
            poly_visible.resize(num_polys);
            for (int i = prev_num_labels; i < num_polys; ++i) {
                poly_parents[i] = -1;
                poly_visible[i] = unordered_set<int>();
            }
        }

        void save_poly_labels_parallel(const char *save_filename) {
            cout << "Saving poly labels in parallel..." << endl;
            {
...
#pragma omp parallel
                {
                    ...
                    const int node_count = poly_labels.size();
...
//                    cout << thread_id << endl;
                    ...
                    int size = poly_labels.size();
                    ofs.write((const char *) &size, sizeof(size));
                    for (int source_node = node_begin; source_node < node_end; ++source_node) {
                        int visible_size = poly_labels[source_node].hub_labels.size();
                        //int p_visible_size = poly_labels[v].p_hub_labels.size();

                        ofs.write((const char *) &visible_size, sizeof(visible_size));
                        //ofs.write((const char*)&p_visible_size, sizeof(p_visible_size));

                        ofs.write((const char *) &poly_labels[source_node].poly, sizeof(poly_labels[source_node].poly));

                        for (int i = 0; i < poly_labels[source_node].hub_labels.size(); ++i) {
                            int temp_size = poly_labels[source_node].hub_labels[i].convex_labels.size();
                            ofs.write((const char *) &poly_labels[source_node].hub_labels[i].hub_id,
                                      sizeof(poly_labels[source_node].hub_labels[i].hub_id));
                            ofs.write((const char *) &temp_size, sizeof(temp_size));
                            for (int j = 0; j < temp_size; ++j) {
                                ofs.write(
                                        (const char *) &poly_labels[source_node].hub_labels[i].convex_labels[j].convex_vertex,
                                        sizeof(poly_labels[source_node].hub_labels[i].convex_labels[j].convex_vertex));
                                ofs.write(
                                        (const char *) &poly_labels[source_node].hub_labels[i].convex_labels[j].distance,
                                        sizeof(poly_labels[source_node].hub_labels[i].convex_labels[j].distance));
                                ofs.write(
                                        (const char *) &poly_labels[source_node].hub_labels[i].convex_labels[j].visibility,
                                        sizeof(poly_labels[source_node].hub_labels[i].convex_labels[j].visibility));
                                ofs.write(
                                        (const char *) &poly_labels[source_node].hub_labels[i].convex_labels[j].predecessor,
                                        sizeof(poly_labels[source_node].hub_labels[i].convex_labels[j].predecessor));
                            }
                            ofs.write((const char *) &poly_labels[source_node].hub_labels[i].min_lower_bound,
                                      sizeof(poly_labels[source_node].hub_labels[i].min_lower_bound));
                        }
                    }
                    ofs.close();
                }
        ...
        void save_adjacent_list(const char *save_filename) {
            ...
            for (int i = 0; i < poly_labels.size(); i++) {
                vector<Hub_label>& hub_labels = poly_labels[i].hub_labels;
                if (hub_labels.size() == 0 ||
                    hub_labels[hub_labels.size() - 1].hub_id != numOfVertices) {
                    hub_labels.push_back(Hub_label{ numOfVertices });
                }
            }
            for (const auto &g_label: poly_labels) {
                poly_label_begin.push_back(g_label_start);
            ...
            poly_label_begin.shrink_to_fit();
            poly_parents.shrink_to_fit();
            poly_visible.shrink_to_fit();
            ...
            ofs.write((const char*)&lambda, sizeof(lambda));
            ofs.write((const char*)&compression, sizeof(compression));
            ofs.write((const char*)&cost_ratio, sizeof(cost_ratio));
            ofs.write((const char*)&store_visibility, sizeof(store_visibility));
            ofs.write((const char*)&all_fully_visible, sizeof(all_fully_visible));
            ...
            int poly_label_size = poly_label_begin.size();
            ofs.write((const char *) &poly_label_size, sizeof(poly_label_size));
            for (int i = 0; i < poly_label_size; ++i) {
                ofs.write((const char *) &poly_label_begin[i], sizeof(poly_label_begin[i]));
                if (i == (poly_label_size - 1))
                    continue;
                if (!store_visibility) {
                    ofs.write((const char*)&poly_parents[i], sizeof(poly_parents[i]));
                }
                if (!store_visibility && !all_fully_visible) {
                    unordered_set<int>& visible_set = poly_visible[i];
                    int num_visible = visible_set.size();
                    ofs.write((const char*)&num_visible, sizeof(num_visible));
                    for (std::unordered_set<int>::iterator itr = visible_set.begin(); itr != visible_set.end(); ++itr) {
                        int visible_vertex = *itr;
                        ofs.write((const char*)&visible_vertex, sizeof(visible_vertex));
                    }
                }
            }

            int num_polys = mesh_poly_parents.size();
            ofs.write((const char*)&num_polys, sizeof(num_polys));
            for (int i = 0; i < num_polys; i++) {
                ofs.write((const char*)&mesh_poly_parents[i], sizeof(mesh_poly_parents[i]));
            }
        ...
        void load_adjacent_list(const char *load_filename) {
            ...
            ifs.read((char*)&compression, sizeof(compression));
            ifs.read((char*)&cost_ratio, sizeof(cost_ratio));
            ifs.read((char*)&store_visibility, sizeof(store_visibility));
            ifs.read((char*)&all_fully_visible, sizeof(all_fully_visible));
            int hub_label_size;
            ...
            int poly_label_size;
            ifs.read((char *) &poly_label_size, sizeof(poly_label_size));
            poly_label_begin.resize(poly_label_size);
            if (!store_visibility) {
                poly_parents.resize(poly_label_size - 1);
                poly_visible.resize(poly_label_size - 1);
            }
            for (int i = 0; i < poly_label_size; ++i) {
                unsigned &g = poly_label_begin[i];
                ...
                if (!store_visibility && !all_fully_visible) {
                    unordered_set<int>& visible_set = poly_visible[i];
                    int num_visible;
                    ifs.read((char*)&num_visible, sizeof(num_visible));
                    aux_space += sizeof(num_visible);
                    for (int j = 0; j < num_visible; ++j) {
                        int visible_vertex;
                        ifs.read((char*)&visible_vertex, sizeof(visible_vertex));
                        visible_set.insert(visible_vertex);
                        aux_space += sizeof(visible_vertex);
                    }
                }
            }

            int num_polys;
            ifs.read((char*)&num_polys, sizeof(num_polys));
            mesh_poly_parents.resize(num_polys);
            for (int i = 0; i < num_polys; ++i) {
                int& mesh_poly_parent = mesh_poly_parents[i];
                ifs.read((char*)&mesh_poly_parent, sizeof(mesh_poly_parent));
                aux_space += sizeof(mesh_poly_parent);
            }
            ...
            for (int i = 0; i < convex_vertex_size; ++i) {
                ...
                if (store_visibility) {
                    ...
                    via_space += sizeof(label.visibility);
                }

                via_space += sizeof(label.convex_vertex);
                via_space += sizeof(label.distance);
                via_space += sizeof(label.predecessor);
            }
            ...
            for(int i = 0; i < hub_lower_size; ++i){
                ...
                hub_space += sizeof(lower_bound);
            }
            if (!store_visibility) {
                vector<vector<int>> all_hub_indices(poly_label_size - 1);
                vector<vector<int>> all_hub_ids(poly_label_size - 1);
                for (int i = 0; i < (poly_label_size - 1); ++i) {
                    const unsigned& start_begin = get_poly_labels_begin(i);
                    for (unsigned j = start_begin; ; j += 2) {
                        int hub_id = get_hub_id(j);
                        if (hub_id == numOfVertices)
                            break;
                        all_hub_indices[i].push_back(j);
                        all_hub_ids[i].push_back(hub_id);
                    }
                }
                hub_parents.resize(hub_lower_size);
                for (int i = 0; i < (poly_label_size - 1); ++i) {
                    const unsigned& start_begin = get_poly_labels_begin(i);
                    for (unsigned j = start_begin; ; j += 2) {
                        int hub_id = get_hub_id(j);
                        if (hub_id == numOfVertices)
                            break;
                        hub_parents[j / 2] = -1;
                        int cur_parent = poly_parents[i];
                        while (cur_parent != -1) {
                            vector<int>& hub_ids = all_hub_ids[cur_parent];
                            vector<int>::iterator hub_it = std::lower_bound(
                                hub_ids.begin(), hub_ids.end(), hub_id);
                            if (hub_it == hub_ids.end() || hub_id < *hub_it)
                            {
                                cur_parent = poly_parents[cur_parent];
                                continue;
                            }
                            vector<int>& hub_indices = all_hub_indices[cur_parent];
                            hub_parents[j / 2] = hub_indices[hub_it - hub_ids.begin()];
                            break;
                        }
                    }
                }
            ...
            poly_parents.shrink_to_fit();
            poly_visible.shrink_to_fit();
            hub_parents.shrink_to_fit();
            convex_label_set.shrink_to_fit();
            hub_lower_bound.shrink_to_fit();
        }

        void load_poly_labels_parallel(const char *load_filename) {
            cout << "loading poly labels in parallel..." << endl;
            //load poly label size of the first poly label file
            string file_name = load_filename;
            string first_file = file_name + "_0";
            ifstream ifs(first_file);
            int isize = 0;
            ifs.read((char *) &isize, sizeof(isize));
            poly_labels.resize(isize);
            {
                printf("Using %d threads\n", omp_get_max_threads());
#pragma omp parallel
                {
...
//                    cout << thread_id << endl;
                    ...
                    cout << filename << endl;
                    //because all the poly label files will write the size ofpoly label, we could just throw it away and do nothing with it
                    int temp_size = 0;
                    ifs.read((char *) &temp_size, sizeof(temp_size));
                    for (int source_node = node_begin; source_node < node_end; ++source_node) {
                        Poly_label &l = poly_labels[source_node];
                        int visible_size;
                        ifs.read((char *) &visible_size, sizeof(visible_size));
                        l.hub_labels.resize(visible_size);
                        ifs.read((char *) &l.poly, sizeof(l.poly));
                        if (visible_size > 0) {
...
//
        ...
        inline int get_poly_index(Point& p) {
            const PointLocation& loc = get_point_location_in_search(p, mp, false);
            return loc.poly1;
        }


        const unsigned& get_poly_labels_begin(const unsigned& poly_index){
            assert(poly_index <= poly_label_begin.size());
            return poly_label_begin[poly_index];
        }

        const unsigned& get_poly_labels_end(const unsigned& poly_index){
            assert(poly_index +1 <= poly_label_begin.size());
            return poly_label_begin[poly_index+1];
        }
...
//            assert(hub_label_index*2+1 <= hub_label_begin.size());
        ...
        const int& get_hub_parent(const unsigned& hub_label_index) {
            return hub_parents[hub_label_index/2];
        }
