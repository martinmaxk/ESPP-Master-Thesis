...
//                                        }
    ...
    string query_output = "dataset/result/query/grid" + to_string(grid_size) + "/" + dir_name + ".csv";
    ...
    if (!did_file_exist)
        q_file << "Map,NumVertices,M-CdtRegions,VisTime,PointLocationTime,ViaLabelTime,TotalTime,ViaLabelsCheckedAvg,VisibleChecksAvg,ViaLabelsPrunedAvg,NumViaLabels,HubSpaceMB,ViaSpaceMB,TotalSpaceMB\n";
    q_file << std::fixed << setprecision(8) << map_name << "," << num_vertices << "," << 
        ...
        ebhl->convex_label_set.size() << "," << (ebhl->hub_space / mb_div) << "," << (ebhl->via_space / mb_div) << "," << 
        ((ebhl->hub_space + ebhl->via_space) / mb_div) << std::endl;
    q_file.close();
